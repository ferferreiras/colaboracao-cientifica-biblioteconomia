<ul>		<?php
		...
		echo "<h4> Processado ".$arquivo."</h4>";
			...
			$file = array();
			// Leitura do arquivo
			if (($handle = fopen("CapLivrosARS2/".$arquivo, "r")) !== FALSE) {
			...
			foreach($arr as $pair){
				...
				if($res == false){
					if ($mysqli->query('INSERT INTO editora (nome) VALUES ("'.$editora.'")')) {
						...
						echo 'Nao foi possivel adicionar o evento ('.$editora.'), saindo com erro : '.$mysqli->error.'<br><br>';
					}
				}elseif(is_int($res)){
					$editoraID = $res;
				}else{
				...
				if($editoraID != null){
					...
					if ($mysqli->query('INSERT INTO capituloLivro (idEditora,ano) VALUES ("'.$editoraID.'",'.$ano.')')) {
						$capLivroID = $mysqli->insert_id;
					}else{
						echo 'Nao foi possivel adicionar o capituloDeLivro com a Editora ('.$editoraID.'), saindo com erro : '.$mysqli->error.'<br><br>';
					}
					...
					foreach ($pair['autores'] as $autor){
						...
						if($pessoaID != null && $capLivroID != null){
							$mysqli->query('INSERT INTO pessoaCapituloLivro (idPessoa,idCapituloLivro) VALUES ('.$pessoaID.','.$capLivroID.')');
						}
